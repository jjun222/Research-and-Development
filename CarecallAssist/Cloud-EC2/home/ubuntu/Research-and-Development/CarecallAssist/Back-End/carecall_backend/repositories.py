...
def telemetry_to_dict(record: TelemetryRecord) -> dict[str, Any]:
    return {field: getattr(record, field) for field in TELEMETRY_FIELDS}


def heartbeat_to_dict(record: DeviceHeartbeatRecord) -> dict[str, Any]:
    return {field: getattr(record, field) for field in HEARTBEAT_FIELDS}
    ...
    *,
    ...
    stale_ids = db.scalars(
        ...
        .where(
            ...
            TelemetryRecord.body_part == record.body_part,
        )
    ...
    if stale_ids:
        db.execute(
            delete(TelemetryRecord).where(TelemetryRecord.id.in_(stale_ids))
        )

    db.commit()
    db.refresh(record)
    return telemetry_to_dict(record)
...
def upsert_device_heartbeat(
    ...
    record = db.get(DeviceHeartbeatRecord, data["device_id"])

    if record is None:
        record = DeviceHeartbeatRecord(**data)
        db.add(record)
    else:
    ...
    db.commit()
    db.refresh(record)
    return heartbeat_to_dict(record)
