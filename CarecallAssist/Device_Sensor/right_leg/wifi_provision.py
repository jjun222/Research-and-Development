"""Jetson-to-limb Wi-Fi provisioning for MicroPython.

The module uses only MicroPython built-ins. It connects to the Jetson setup AP,
downloads this limb's saved uplink credentials through the authenticated device API,
...
TOKEN_PLACEHOLDER = "PASTE_JETSON_DEVICE_PROVISION_TOKEN_HERE"
...
def _fetch_credentials():
    path = "/api/device/wifi?device_id={}".format(DEVICE_ID)
...
def connect_wifi_forever(wlan=None):
    ...
    if hasattr(network, "hostname"):
        try:
            ...
            print("[WIFI] hostname warning: {}".format(error))
