...
try:
    ...
    raise OSError("Pico W 전용 MicroPython 펌웨어를 설치하세요.")
...
PWM_FREQ = const(1000)
ENA = PWM(Pin(15)); IN1 = Pin(14, Pin.OUT); IN2 = Pin(13, Pin.OUT)
ENB = PWM(Pin(12)); IN3 = Pin(11, Pin.OUT); IN4 = Pin(10, Pin.OUT)
...
def set_power_ratio(ratio):
    duty = 0 if ratio <= 0 else 65535 if ratio >= 1.0 else int(65535 * ratio)
    ENA.duty_u16(duty); ENB.duty_u16(duty)

def stop_all():
    set_power_ratio(0.0); IN1.value(0); IN2.value(0); IN3.value(0); IN4.value(0)

_state = {"active": False, "end_ms": 0, "on_ms": 300, "off_ms": 300, "next_toggle_ms": 0, "vibrating": False, "intensity": 0.8}
...
def start_fire_alert(duration_ms=10000, on_ms=400, off_ms=200, intensity=0.85):
    set_motor_forward()
    now = ticks_ms()
    _state.update({"active": True, "end_ms": time.ticks_add(now, int(duration_ms)), "on_ms": int(on_ms), "off_ms": int(off_ms), "next_toggle_ms": now, "vibrating": False, "intensity": max(0.0, min(1.0, float(intensity)))})

def stop_pattern():
    _state["active"] = False; stop_all()
...
def pattern_tick():
    ...
    if time.ticks_diff(now, _state["next_toggle_ms"]) >= 0:
        if _state["vibrating"]:
            set_power_ratio(0.0); _state["vibrating"] = False; _state["next_toggle_ms"] = time.ticks_add(now, _state["off_ms"])
        else:
            set_motor_forward(); set_power_ratio(_state["intensity"]); _state["vibrating"] = True; _state["next_toggle_ms"] = time.ticks_add(now, _state["on_ms"])

def load_wifi_config():
    if CONFIG_PATH not in os.listdir(): return None
    try:
        with open(CONFIG_PATH, "r") as f: return ujson.loads(f.read())
...
def save_wifi_config(ssid, pw, broker_ip=None):
    ...
    with open(CONFIG_PATH, "w") as f: f.write(ujson.dumps(cfg))
...
def radio_reset():
    ...
    try:
        ...
        pass
    if wlan is None: wlan = network.WLAN(network.STA_IF)
    try: wlan.disconnect()
...
def try_connect_wifi(ssid, pw, force_reset=True):
    ...
    try: wlan.connect(ssid, pw)
    except Exception as e:
        print("❌ wlan.connect 실패:", e); return False
    attempt = 0
    while not wlan.isconnected() and attempt < WIFI_RETRY_MAX:
        attempt += 1; time.sleep_ms(WIFI_RETRY_WAIT_MS)
    if not wlan.isconnected():
        print("❌ Wi-Fi 연결 실패"); return False
    print("✅ Wi-Fi 연결 완료:", wlan.ifconfig()); led_blink(3,80,80); return True

def connect_wifi_from_config(force_reset=True):
    global MQTT_BROKER
    cfg = load_wifi_config()
    if cfg:
        ssid = cfg.get("ssid"); pw = cfg.get("password"); broker = cfg.get("broker") or DEFAULT_BROKER_IP
        if ssid and pw:
            MQTT_BROKER = broker
            if try_connect_wifi(ssid, pw, force_reset): 
                print("🌐 config로 Wi-Fi 연결 OK, broker =", MQTT_BROKER); return True
...
def probe_broker(ip, port=1883, timeout_s=2):
    ...
    try:
        ...
        s = socket.socket(); s.settimeout(timeout_s); s.connect(addr); return True
    except Exception as e:
        print("⚠️ broker reachability check 실패:", e); return False
    finally:
        try:
            if s: s.close()
        except Exception: pass
...
def on_msg(topic, msg):
    ...
    try:
        ...
        print("JSON parse error:", e, msg); return
...
def mqtt_connect_and_sub():
    ...
    c.set_callback(on_msg); c.connect()
    for t in SUB_TOPICS:
        c.subscribe(t, qos=1); print("SUB:", t)
    client = c; return c
...
def main():
    ...
    stop_all(); led_off(); led_blink(2,100,100)
    ...
    while True:
        try:
            ...
            client = None; stop_pattern(); led_blink(1,200,200)
