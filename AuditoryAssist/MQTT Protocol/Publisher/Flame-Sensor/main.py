...
def _candidate_broadcast_addresses():
    """
    255.255.255.255와 현재 서브넷 브로드캐스트 주소를 함께 시도한다.
    일부 공유기/펌웨어에서 255.255.255.255 broadcast가 제한될 수 있어
    192.168.x.255 형태도 같이 보낸다.
    """
    ...
    try:
        if wlan is not None and wlan.isconnected():
            ...
            parts = ip.split(".")
            if len(parts) == 4:
                subnet_broadcast = "%s.%s.%s.255" % (parts[0], parts[1], parts[2])
    ...
    try:
        ...
        for bcast in _candidate_broadcast_addresses():
            try:
                ...
                print("⚠️ discovery broadcast 실패:", bcast, e)
...
def ensure_mqtt_broker():
    """
    MQTT 연결 직전에 Broker가 비어 있으면 config/discovery 순서로 다시 확인한다.
    """
    global MQTT_BROKER
...
def rediscover_mqtt_broker():
    """
    저장된 broker가 오래되어 연결에 실패했을 때 사용한다.
    config broker를 다시 쓰지 않고 UDP Discovery를 우선 시도한다.
    """
    global MQTT_BROKER
...
def mqtt_connect():
    ...
    for attempt in range(2):
        try:
            ...
            if rediscover_mqtt_broker() and MQTT_BROKER and MQTT_BROKER != old_broker:
